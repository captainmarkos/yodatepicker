...
/* jshint unused: false */
var yolib = {
  get_max_date: function get_max_date(param_max_date) {
    ...
    if (param_max_date instanceof Date) {
      return param_max_date;
    }
  ...
  get_min_date: function get_min_date(param_min_date) {
    // Return a date object set to min_date.
    // Acceptable parameter formats: 0, 3M, 6M, 9M, 1Y, 2Y, * (infinity)
    var date = new Date();
    var month = date.getMonth();
    var year = date.getFullYear(); // If a Date object is passed in, use that.
  ...
  get_dow_names: function get_dow_names(locale, dow_heading) {
    var names = [];
  ...
  get_month_names: function get_month_names(locale) {
  ...
  dow_single_names: function dow_single_names(locale) {
    var names = [];
  ...
  dow_short_names: function dow_short_names(locale) {
    var names = [];
  ...
  dow_full_names: function dow_full_names(locale) {
    var names = [];
  ...
  days_in_month: function days_in_month(month_num, full_year) {
    // Jan == 0, Feb == 1, Mar == 2, ...
    if (month_num === 0 || month_num === 2 || month_num === 4 || month_num === 6 || month_num === 7 || month_num === 9 || month_num === 11) {
      return 31;
    } else if (month_num === 3 || month_num === 5 || month_num === 8 || month_num === 10) {
      return 30;
    }

    return yolib.leap_year(full_year) ? 28 : 29;
  },
  leap_year: function leap_year(yr) {
...
var yoconfig = {
  ...
  prepare: function prepare(opts) {
    this.config = {
      ...
      ondateselected_callback: opts.onDateSelected instanceof Function ? opts.onDateSelected : null,
      // User defined function to be called when closing yodatepicker.
      onclose_callback: opts.onClose instanceof Function ? opts.onClose : null,
    ...
    this.config.today = new Date(this.config.currdate.getFullYear(), this.config.currdate.getMonth(), this.config.currdate.getDate()); // array of month names

    this.config.month_names = yolib.get_month_names(this.config.locale); // array of day of week names

    this.config.day_names = yolib.get_dow_names(this.config.locale, this.config.dow_heading); // Keeps track of the month the datepicker is on and will
    // not go past the min_date month (if set).

    var dp_display_date = this.config.current_start_date ? this.config.current_start_date : this.config.currdate;
    this.config.mn = dp_display_date.getTime() < this.config.min_date.getTime() ? this.config.min_date.getMonth() : dp_display_date.getMonth(); // Keeps track of the year the datepicker is on and will
    // not go past the min_date year (if set).

    this.config.yy = dp_display_date.getTime() < this.config.min_date.getTime() ? this.config.min_date.getFullYear() : dp_display_date.getFullYear(); // Set flag, tiggers the datepicker to close on selecting a day.

    this.config.close_onselect = this.config.close_onselect === undefined ? true : this.config.close_onselect; // Limit the number of months to display for a multi-month datepicker.

    this.config.months_to_display = this.config.months_to_display > this.config.MAX_CALENDARS ? this.config.MAX_CALENDARS : this.config.months_to_display; // This feature is only applicable when close_onselect is false and
    ...
    this.config.date_range = this.config.use_date_range ? {
      start: true,
      stop: false
    } : null;
    return this.config;
...
yoconfig.dependencies.push('yolib.js');
/* jshint strict: true */
